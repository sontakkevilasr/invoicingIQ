...
<form id="invoiceForm" method="POST"
    ...
    <div class="inv-parties">
        <div>
            ...
            <input type="hidden" id="f_cust_email" value="<?php echo e($isEdit && $invoice->customer ? $invoice->customer->email : ''); ?>">
...
<?php if($isEdit): ?>
<form id="emailSendForm" method="POST" action="<?php echo e(route('invoices.email', $invoice)); ?>" style="display:none;">
    <?php echo csrf_field(); ?>
...
/* ── Email modal ── */
const EMAIL_ENABLED = <?php echo e(($s['email_enabled']??'0')==='1' ? 'true' : 'false'); ?>;
const EMAIL_SUBJECT = <?php echo json_encode($s['email_subject'] ?? 'Invoice {invoice_number} from {company_name}', 15, 512) ?>;
const EMAIL_BODY    = <?php echo json_encode($s['email_body'] ?? '<p>Dear {customer_name}, </p><p>Please find attached <strong>Invoice {invoice_number}</strong> from <strong>{company_name}</strong>.</p><p><strong>Amount:</strong> {amount}<br><strong>Due Date:</strong> {due_date}</p><p>The invoice PDF is included as a compressed attachment.</p><p>Regards, <br>{company_name}</p>') ?>;
const COMPANY_NAME  = <?php echo json_encode($s['company_name'] ?? '', 15, 512) ?>;
